...
function App() {
  ...
  const scrollToEvent = (timestamp) => {
    const playerElement = document.getElementById("player");
    if (playerElement) {
      playerElement.scrollLeft = timestamp; // Assuming timestamp is the scroll position you want to go to
    }
  };

  const handleRowClick = (timestamp) => {
    scrollToEvent(timestamp);
  };
        ...
        <table>
          ...
          <tbody>
            {events.map((event, index) => (
              <tr key={index} onClick={() => handleRowClick(event.timestamp)}>
                <td>{`Event ${index + 1}`}</td>
              </tr>
            ))}
          </tbody>
